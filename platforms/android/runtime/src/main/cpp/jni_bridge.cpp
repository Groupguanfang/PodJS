...
struct Host { PodRuntime* runtime{}; std::unique_ptr<VulkanRenderer> renderer; std::vector<PodTouch> touches; int32_t rotary{}; };
...
extern "C" JNIEXPORT jlong JNICALL Java_dev_podjs_runtime_PodRuntimeView_nativeCreate(
  ...
  host->renderer = std::make_unique<VulkanRenderer>(window, width, height); ANativeWindow_release(window);
  env->ReleaseStringUTFChars(target, t); env->ReleaseStringUTFChars(dataDir, d);
  if (!host->runtime || !host->renderer->valid()) { fail(env, pod_runtime_last_error()); return 0; }
...
extern "C" JNIEXPORT void JNICALL Java_dev_podjs_runtime_PodRuntimeView_nativeResize(JNIEnv* env,jclass,jlong p,jobject s,jint w,jint ht){auto*h=reinterpret_cast<Host*>(p);if(!h)return;auto*n=ANativeWindow_fromSurface(env,s);h->renderer->replaceWindow(n,w,ht);ANativeWindow_release(n);}
...
extern "C" JNIEXPORT void JNICALL Java_dev_podjs_runtime_PodRuntimeView_nativeInput(JNIEnv* env,jclass,jlong p,jintArray ids,jfloatArray xy,jint count,jint rotary){auto*h=reinterpret_cast<Host*>(p);if(!h)return;std::vector<jint> id(count);std::vector<jfloat> pos(count*2);if(count){env->GetIntArrayRegion(ids,0,count,id.data());env->GetFloatArrayRegion(xy,0,count*2,pos.data());}h->touches.resize(count);for(int i=0;i<count;i++)h->touches[i]={(uint32_t)id[i],pos[i*2],pos[i*2+1]};h->rotary+=rotary;}
extern "C" JNIEXPORT void JNICALL Java_dev_podjs_runtime_PodRuntimeView_nativeFrame(JNIEnv*,jclass,jlong p){auto*h=reinterpret_cast<Host*>(p);if(!h)return;PodInputFrame in{sizeof(in),0,0,h->touches.data(),(uint32_t)h->touches.size(),h->rotary,0};h->rotary=0;if(pod_runtime_frame(h->runtime,&in)==0){PodDrawList list{};if(!pod_runtime_snapshot(h->runtime,&list)&&list.changed){bool submitted=h->renderer->submit(list.words,list.word_count,list.content_hash);__android_log_print(ANDROID_LOG_DEBUG,"PodJS","frame=%llu words=%zu hash=%016llx submitted=%d",(unsigned long long)list.frame_number,list.word_count,(unsigned long long)list.content_hash,submitted);}}}
