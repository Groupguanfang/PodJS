...
class Storage {
  ...
  static int file(int parent, const std::string& name) {
    Fd input(openat(parent, name.c_str(), O_RDONLY | O_NOFOLLOW | O_NONBLOCK | O_CLOEXEC));
    if (input.value < 0 && errno == ENOENT) return -1;
    struct stat st{}; require(input.value >= 0 && fstat(input.value, &st) == 0 && pod_store::privateFile(st), "Unsafe incoming file");
  ...
  static bool verified(int parent, const std::string& name, size_t size, const std::string& hash) {
    Fd input(file(parent, name)); if (input.value < 0) return false;
