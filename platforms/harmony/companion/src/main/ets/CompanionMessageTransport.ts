...
export class CompanionMessageTransport {
  ...
  driveOutgoing(): void {
    ...
    if (!this.automaticOutgoing) {
      ...
      try {
        if (this.stateSource !== null) this.unsubscribeState = this.stateSource.subscribe(() => this.scheduleOutgoing());
  ...
  private scheduleOutgoing(): void {
    ...
    Promise.resolve().then(async () => {
      while (!this.closed && this.outgoingRequested) {
        this.outgoingRequested = false;
        if (this.channels !== null) await this.sendState();
        if (!this.closed) await this.sendNext();
      }
  ...
  acknowledge(delivery: CompanionIncomingMessage): Promise<void> {
